...
% _____________________________________________________________________________
% Ricker, from Koene et al. fig 3 add/rm, 8 Hz Ricker, time (0:0.015:0.4)
tmin = 0; tmax = 0.4; doplot = 1;
type_wavelet = 0; % 0 or 1
% for series
halfordermax = 4; % up to dt^(2*halfordermax)
nhextra = 3; % extra points for stencil to suppress noise (2*nhextra added)
npextra = 48; % ?? has to be made precise

switch type_wavelet
  case 1
    Tw = 0.18; ww = @(t0) wavelet16(t0-(tmin+tmax-Tw)/2,Tw,0)';
...
for halforder=1:halfordermax
  % add dispersion
  ac2 = dispcor_series(wt0,t0-tmin,nsub,'f',halforder,reduce_order,...
    nhextra,npextra);
  % (a,t,dt,isub,halforder,cdir,reduce_order,ndermax,nhextra,npextra,dotst1,pltinfo)
  ws1 = wt0+ac2;
  % remove dispersion
  
  bc2 = dispcor_series(ws1,t0-tmin,nsub,'i',halforder,reduce_order,...
    nhextra,npextra);
  ws2 = ws1+bc2;
